...
#     case32 case32_day17 scans slice_0025_266_266_1.50_1.50.png
#     case32_day17_slice_0025
    ################################################################
    df3 = pd.DataFrame()
    ...
    for f in img_files:
        temp_f = f.split('/')
        ids.append(temp_f[1]+'_'+temp_f[3][:-22])
        cases.append(int(temp_f[0][4:]))
    df3['img_files'] = img_files
    df3['id'] = ids
    df3['case'] = cases
    
    
    df = pd.DataFrame()
    df['segmentation'] = meta_df.segmentation
    df['id'] = meta_df.id
