...
class Trainer:
    def __init__(
        ...
        self.create_dataloaders(num_workers, subset, conf.fold_ids)
    ...
    def create_dataloaders(self, num_workers, subset, fold_id = 0):
        conf = self.conf
        ...
        ############################## Jaafar  #######################################
        skf = StratifiedGroupKFold(n_splits=conf.n_folds, shuffle=True, random_state=43)
...
#         display(df.groupby(['fold','empty'])['id'].count())
        ...
        val_df = df.query("fold==@fold_id").reset_index(drop=True)
...
#             result += func(outputs, labels)
        result = 1.0 * self.loss_funcs[0](outputs, labels) + 0.0 * self.loss_funcs[1](outputs, labels)
        return result
